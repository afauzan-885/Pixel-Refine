...
from pixel_refine_desktop.core.algorithm.alignment.AKAZE import AKAZEAlgorithm
...
from config import ALGORITHM_PARAMETER_SETTINGS_FILE, CONFIG_DIR, GENERAL_SETTINGS_FILE

# --- Fungsi Helper (Sama seperti di ORB) ---
...
def save_akaze_config(config):
    ...
    all_params = {}
    try:
        # Baca file konfigurasi yang ada untuk mempertahankan bagian lain
        if os.path.exists(config_filename):
            with open(config_filename, "r") as f:
                all_params = json.load(f)
    except Exception as e:
        print(f"Warning: Could not read existing config file '{config_filename}' before saving AKAZE: {e}")

    # Timpa atau tambahkan bagian AKAZE
...
# --- Fungsi Utama UI AKAZE ---
def get_akaze_page():
    # --- LANGKAH 1: Initial Load & Sync ---
    try:
        all_params = {}
        akaze_section_exists = False
        if os.path.exists(ALGORITHM_PARAMETER_SETTINGS_FILE):
            with open(ALGORITHM_PARAMETER_SETTINGS_FILE, "r") as f:
                all_params = json.load(f)
            # Cek apakah 'AKAZE' ada dan merupakan dictionary
            if "AKAZE" in all_params and isinstance(all_params.get("AKAZE"), dict):
                akaze_section_exists = True

        if not akaze_section_exists:
            akaze_params = load_akaze_config()
            general_settings = _load_general_settings()
            use_multicore_setting = general_settings.get("multi_core_cpu", True)

            akaze_params["use_multi_core"] = use_multicore_setting
            save_akaze_config(akaze_params)
            akaze_config = akaze_params

        else:
            akaze_config = load_akaze_config()
            if "use_multi_core" not in akaze_config:
                 general_settings = _load_general_settings()
                 akaze_config["use_multi_core"] = general_settings.get("multi_core_cpu", True)
                 save_akaze_config(akaze_config)

    except (IOError, json.JSONDecodeError) as e:
        print(f"Error during initial check/load of {ALGORITHM_PARAMETER_SETTINGS_FILE}: {e}. Using default values for AKAZE UI.")
        akaze_config = load_akaze_config()
   
    # --- Mulai Setup UI menggunakan akaze_config ---
    page = QWidget()
    ...
    # --- LANGKAH 2: UI Element Storage ---
    sliders = {}
    value_labels = {}
    param_formatters = {}
    combos = {}
    toggles = {}
    folder_data = {'selected_path': akaze_config.get("align_folder", "")}
    ...
    def save_current_settings():
        ...
        # 2. Ambil config AKAZE terbaru sebagai basis
        current_loaded_config = load_akaze_config()
        akaze_params = current_loaded_config.copy()
        ...
        for label_key, slider_widget in sliders.items():
            ...
            config_key = None
            fmt_func = None
            for p_label, _, _, _, _, p_ckey, mult, p_fmt, _ in params_config: # Gunakan list params_config
                 if p_label == label_key:
                     config_key = p_ckey
                     fmt_func = p_fmt
                     break
            if config_key:
                if config_key == "akaze_threshold": akaze_params["akaze_threshold"] = current_value / 10000.0
                elif config_key == "akaze_nOctaves": akaze_params["akaze_nOctaves"] = current_value
                elif config_key == "akaze_nOctaveLayers": akaze_params["akaze_nOctaveLayers"] = current_value
                elif config_key == "ratio_threshold": akaze_params["ratio_threshold"] = current_value / 100.0
                elif config_key == "ransacThreshold": akaze_params["ransacThreshold"] = current_value / 10.0
        ...
        if 'transformation' in combos:
            akaze_params["transformation"] = combos['transformation'].currentText()

        # 5. Update dari Toggle Buttons
        if 'keep_edges' in toggles:
            akaze_params["keep_edges"] = toggles['keep_edges'].isChecked()
        if 'enable_cropping' in toggles:
             akaze_params["enable_cropping"] = toggles['enable_cropping'].isChecked()
        if 'save_align' in toggles:
             akaze_params["save_align"] = toggles['save_align'].isChecked()
        if 'command_save_to_hd5f' in toggles:
             akaze_params["command_save_to_hd5f"] = toggles['command_save_to_hd5f'].isChecked()

        # 6. Update Align Folder Path
        try:
            normalized_path = os.path.normpath(folder_data['selected_path']).replace("\\", "/")
            akaze_params["align_folder"] = normalized_path
        except TypeError: 
            akaze_params["align_folder"] = "" 
            
        # 7. Update use_multi_core
        akaze_params["use_multi_core"] = use_multicore_setting
    ...
    params_config = [
        (language_config.AKAZE_THRESHOLD_LABEL, 1, 100, 1, akaze_config.get("akaze_threshold", 0.001), "akaze_threshold", 10000, lambda v: f"{v/10000:.4f}", language_config.AKAZE_THRESHOLD_DESCRIPTION),
        (language_config.AKAZE_OCTAVE_LABEL, 1, 8, 1, akaze_config.get("akaze_nOctaves", 4), "akaze_nOctaves", 1, str, language_config.AKAZE_OCTAVE_DESCRIPTION),
        (language_config.AKAZE_LAYER_LABEL, 1, 10, 1, akaze_config.get("akaze_nOctaveLayers", 4), "akaze_nOctaveLayers", 1, str, language_config.AKAZE_LAYER_DESCRIPTION),
        (language_config.AKAZE_RATIO_LABEL, 50, 100, 1, akaze_config.get("ratio_threshold", 0.75), "ratio_threshold", 100, lambda v: f"{v/100:.2f}", language_config.AKAZE_RATIO_DESCRIPTION),
        (language_config.ORB_RANSAC_LABEL, 10, 100, 5, akaze_config.get("ransacThreshold", 5.0), "ransacThreshold", 10, lambda v: f"{v/10:.1f}", language_config.ORB_RANSAC_DESCRIPTION)
    ]

    for label_key, min_v, max_v, step, initial_float_value, config_key, mult, fmt, tip in params_config:
    ...
    transformation_label = QLabel(language_config.ORB_TRANSFORMATION_LABEL) # Ganti jika ada label spesifik AKAZE
    transformation_label.setToolTip(language_config.ORB_TRANSFORMATION_DESCRIPTION) # Ganti jika ada tooltip spesifik AKAZE
    ...
    keep_edges_initial = akaze_config.get("keep_edges", False)
    ...
    toggles['keep_edges'] = keep_edges_button
    ...
    enable_cropping_initial = akaze_config.get("enable_cropping", False)
    ...
    toggles['enable_cropping'] = enable_cropping_button 
    ...
    extra_params_layout_main = QVBoxLayout()
    ...
    save_align_initial = akaze_config.get("save_align", False)
    ...
    command_save_hd5f_initial = akaze_config.get("command_save_to_hd5f", True)
    ...
    def on_align_folder_changed(index):
        selected_text = align_folder_dropdown.itemText(index)

        if selected_text == language_config.SEARCH_SAVE_ALIGN_IMAGE_TO_FOLDER:
            start_dir = folder_data['selected_path'] if folder_data['selected_path'] and os.path.isdir(folder_data['selected_path']) else ""
            folder_path = QFileDialog.getExistingDirectory(None, language_config.SELECT_SAVE_ALIGN_IMAGE_TO_FOLDER, start_dir)

            if folder_path:
                folder_data['selected_path'] = os.path.normpath(folder_path) 
                ...
                align_folder_dropdown.blockSignals(True)
                if folder_data['selected_path']:
                     existing_path_index = align_folder_dropdown.findText(truncate_path(folder_data['selected_path']))
                     if existing_path_index != -1:
                          align_folder_dropdown.setCurrentIndex(existing_path_index)
                     elif align_folder_dropdown.count() > 1 and align_folder_dropdown.itemText(0) != language_config.SEARCH_SAVE_ALIGN_IMAGE_TO_FOLDER:
                          align_folder_dropdown.setCurrentIndex(0)
                     else:
                          align_folder_dropdown.setCurrentIndex(align_folder_dropdown.findText(language_config.SEARCH_SAVE_ALIGN_IMAGE_TO_FOLDER)) # Fallback ke search
                else:
                    search_index = align_folder_dropdown.findText(language_config.SEARCH_SAVE_ALIGN_IMAGE_TO_FOLDER)
                    if search_index != -1:
                         align_folder_dropdown.setCurrentIndex(search_index)
                align_folder_dropdown.blockSignals(False)
    ...
    extra_params_layout_main.addLayout(folder_selection_layout)
    ...
    def on_save_align_toggled(state):
        ...
        if not state: 
            if not command_save_to_hd5f_button.isChecked():
                ...
                command_save_to_hd5f_button.setText(language_config.ACTIVATE_SAVE_ALIGN_TO_PROCESS)
