...
# from pixel_refine_desktop.ui.components.single_page.image_preview_handler import update_preview_panel
...
from pixel_refine_desktop.ui.components.single_page.left_panel import LeftPanel
from pixel_refine_desktop.ui.components.single_page.right_panel import RightPanel
...
def setup_progress_section(layout_instance):
    ...
    layout_instance.process_button = QPushButton(
    ...
    layout_instance.save_as_button = QPushButton(
        language_config.PROGRESS_SECTION_SAVE_BUTTON_TEXT
    )
...
def setup_preview_panel(layout_instance):
    ...
    layout_instance.preview_view = Zoomable(
        layout_instance.preview_scene, layout_instance
    )  # Set parent

    # Style dasar view (bisa juga diatur di dalam Zoomable.__init__)
    layout_instance.preview_view.setStyleSheet(stylesheet.PREVIEW_VIEW_STYLE)

    # Tambahkan view ke layout di LeftPanel
    # --- Pastikan left_panel dan preview_panel_widget ada dan punya layout ---
    if hasattr(layout_instance, "left_panel") and hasattr(
        layout_instance.left_panel, "preview_panel_widget"
    ):
        ...
        if not preview_layout:
            preview_layout = QVBoxLayout(
                preview_panel
            )  # Atau QHBoxLayout sesuai kebutuhan
...
def setup_signals(layout_instance):
    ...
    if hasattr(layout_instance, "process_button"):
        ...
        layout_instance.process_clicked.connect(
            layout_instance.single_process_algorithm
        )
    else:
        pass
