...
from pixel_refine_desktop.ui.views.panorama.Algorithm.Blend.base_blender import (
    BaseBlender,
)
from pixel_refine_desktop.ui.views.panorama.Algorithm.Projection_and_Crop.image_warping import (
    get_warper,
)
from pixel_refine_desktop.ui.views.panorama.Algorithm.Projection_and_Crop.image_warping.base_warper import (
    BaseWarper,
)
...
def load_images(paths: List[str]) -> List[np.ndarray]:
    ...
    for path in paths:
        ...
        if img is None:
            raise IOError(
                f"Gagal membaca atau format tidak didukung untuk gambar: {path}"
            )
...
def detect_features(img, detector, use_multicore=True, num_features=5000):
    ...
    if img is None:
        return [], None
    gray_img = (
        cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) if img.ndim == 3 else img.astype(np.uint8)
    )
    ...
    if noise_level > 600.0:
        ...
        print(
            f"Noise tinggi ({noise_level:.2f}), menerapkan Bilateral Filter (d={d}, sigma={sigma})..."
        )
    ...
    def process_block(i, j):
        ...
        x_end, y_end = min(w, roi_x + roi_w + overlap), min(
            h, y_start + roi_h + overlap
        )  # Koreksi bug kecil
...
def compose_transformations_from_graph(pairwise_matches, n_images, anchor_idx):
    ...
    graph = csr_matrix(
        (weights, (rows, cols)), shape=(n_images, n_images), dtype=np.float64
    )
    ...
    for match in pairwise_matches:
        ...
        try:
            transform_map[(match["dst_idx"], match["src_idx"])] = np.linalg.inv(
                match["T"]
            )
        ...
        for neighbor_idx in list(neighbors_fwd) + list(neighbors_bwd):
            if neighbor_idx not in visited:
                ...
                if T_to_neighbor is not None:
                    final_transforms[neighbor_idx] = (
                        final_transforms[current_idx] @ T_to_neighbor
                    )
                    q.append(neighbor_idx)

    if any(t is None for t in final_transforms):
        return None
...
def _render_single_tile(args: tuple) -> tuple:
    ...
    for image_index in task_info["relevant_indices"]:
        ...
        warped_tile, mask = warper.warp_and_mask_layer(
            img_float, task_info, image_index
        )
        del img_float
    ...
    if not warped_layers_float:
        return (
            task_info["y_start"],
            task_info["x_start"],
            np.zeros(task_info["tile_shape"], dtype=dtype_tile),
        )
...
def render_panorama_tiles(
    ...
    TILE_SIZE = (2048, 2048)
    dtype_out = (
        np.uint8 if save_as_uint8 else (np.float16 if save_as_float16 else np.float32)
    )
    ...
    if output_memmap is None:
        ...
        output_memmap = np.memmap(
            memmap_path, dtype=dtype_out, mode="w+", shape=output_shape
        )
    ...
    all_tasks_args = [
        (task, warper, blender, save_as_uint8, save_as_float16)
                ...
                for y_start, x_start, tile_float in tile_buffer:
                    ...
                    if progress_callback:
                        with progress_lock:
                            ...
                            progress = p_start + (progress_counter / total_tiles) * (
                                p_end - p_start
                            )
                            progress_callback(
                                progress,
                                f"Memproses Tile {progress_counter}/{total_tiles}",
                            )
                output_memmap.flush()
                tile_buffer.clear()
            ...
            for y_start, x_start, tile_float in tile_buffer:
                ...
                if progress_callback:
                    with progress_lock:
                        ...
                        progress_callback(
                            progress, f"Memproses Tile {progress_counter}/{total_tiles}"
                        )
            output_memmap.flush()
            tile_buffer.clear()
    ...
    def worker_func(task_args):
        return _render_single_tile(
            task_args
        )  # kembalikan (y_start, x_start, tile_float)

    with ThreadPoolExecutor(
        max_workers=min(max_in_flight_tiles, os.cpu_count())
    ) as executor:
