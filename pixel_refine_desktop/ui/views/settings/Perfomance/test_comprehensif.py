...
# Path setup to ensure absolute imports work when this standalone script is
# launched directly by the Performance Settings subprocess worker.
...
def run_fast_hardware_test():
    ...
    benchmark_measured_elapsed_ms = 0.0
    skipped_tests = 0
    resident_inputs = []
    ...
    def record(name, valid, timings=None):
        status = "[PASS]" if valid else "[FAIL]"
        timing_text = ""
        if timings:
            timing_text = f" | median_ms={float(np.median(timings)):.3f}"
        print(f"{status} {name}{timing_text}")
        results.append(bool(valid))

    def run_repeated(name, operation, validator):
        """Run one GPU-resident graph four times; discard warm-up timing."""
        nonlocal benchmark_measured_elapsed_ms, skipped_tests
        valid = True
        measured_ms = []
        for iteration in range(benchmark_runs):
            started = time.perf_counter()
            try:
                value = operation()
            except RuntimeError as exc:
                # Quarantine is a capability decision from the canonical
                # resolver, not a benchmark failure. Keep the backend result
                # useful by isolating the unavailable optional graph.
                if "quarantined" in str(exc).lower():
                    skipped_tests += 1
                    ...
                    return None
                raise
            # Inputs are already GPU buffers. Synchronization closes the
            # dispatch before reading the clock, without downloading output.
            taichi_aot.engine.sync()
            elapsed_ms = (time.perf_counter() - started) * 1000.0
            valid = bool(validator(value)) and valid
            if iteration >= benchmark_warmup_runs:
        ...
        benchmark_metrics[name] = {
            ...
            "median_ms": float(np.median(measured_ms)) if measured_ms else None,
        }
        benchmark_measured_elapsed_ms += float(np.mean(measured_ms)) if measured_ms else 0.0
        record(name, valid, measured_ms)
        return value

    try:
        y, x = np.mgrid[:benchmark_height, :benchmark_width]
        gray = ((x * 0.7 + y * 0.3) / 1023.0).astype(np.float32)
        # Upload once, before timing. Every measured operation receives GPU
        # buffers and returns GPU buffers, so host transfer is excluded.
        gray_gpu = taichi_aot.upload(gray)
        flow_gpu = taichi_aot.upload(
            np.zeros((benchmark_height, benchmark_width, 2), dtype=np.float32)
        )
        resident_inputs.extend((gray_gpu, flow_gpu))

        resized = run_repeated(
            "Resize dispatch/readback",
            lambda: taichi_aot.resize(
                gray_gpu,
                (512, 512),
                interpolation=taichi_aot.INTER_CUBIC,
                return_gpu=True,
            ),
            lambda value: getattr(value, "shape", None) == (512, 512),
        )

        blurred = run_repeated(
            "Gaussian native graph",
            lambda: taichi_aot.gaussian_blur(
                gray_gpu, sigma=1.5, return_gpu=True
            ),
            lambda value: getattr(value, "shape", None) == gray.shape,
        )

        gradients = run_repeated(
            "Gradient multi-output lifecycle",
            lambda: taichi_aot.sobel(gray_gpu, return_gpu=True),
            lambda value: (
                isinstance(value, tuple)
                and len(value) == 2
                and value[0].shape == gray.shape
                and value[1].shape == gray.shape
            ),
        )

        edges = run_repeated(
            "Canny native pipeline",
            lambda: taichi_aot.canny_aot(
                gray_gpu,
                low_threshold=50.0,
        ...
        flow_result = run_repeated(
            "Spatial-fusion warp/remap graph",
            lambda: taichi_aot.remap_with_flow(
                gray_gpu,
                flow_gpu,
                benchmark_height,
                benchmark_width,
            ...
            == (benchmark_height, benchmark_width),
        )
        for buffer in resident_inputs:
            release_buffer(buffer)
        resident_inputs.clear()
    ...
    passed = sum(results)
    total = len(results)
    wall_elapsed_ms = (time.perf_counter() - benchmark_started) * 1000.0
    # The persisted benchmark value is GPU-only: host upload, module loading,
    # allocation warm-up, and the first dispatch are intentionally excluded.
    benchmark_elapsed_ms = benchmark_measured_elapsed_ms
    print(
        f">>> Benchmark: {benchmark_width}x{benchmark_height} | "
        f"elapsed_ms={benchmark_elapsed_ms:.3f} | "
        f"megapixels={benchmark_width * benchmark_height / 1_000_000.0:.6f} | "
        f"runs={benchmark_runs} | warmup={benchmark_warmup_runs} | "
        f"gpu_only=1 | wall_elapsed_ms={wall_elapsed_ms:.3f}"
    )
    print(">>> Benchmark Metrics: " + json.dumps(benchmark_metrics, sort_keys=True))
