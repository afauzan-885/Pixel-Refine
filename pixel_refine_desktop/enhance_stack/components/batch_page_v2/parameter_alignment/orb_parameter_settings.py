"""
ORB Parameter Settings - Config Provider
========================================

Backend value provider for ORB parameters.
...
def save_orb_config(config):
    ...
    try:
        ...
        print(
            f"Warning: Could not read existing config file '{config_filename}' before saving ORB: {e}"
        )
...
from PySide6.QtWidgets import (
    ...
    QComboBox,
    QFileDialog,
)  # QPushButton tidak diperlukan lagi
...
def get_orb_page():
    try:
        ...
        print(
            f"Error during initial check/load of {ALGORITHM_PARAMETER_SETTINGS_FILE}: {e}. Using default values for ORB UI."
        )
    ...
    def save_current_settings():
        ...
        for label_key, slider_widget in sliders.items():
            ...
            if config_key:
                ...
                if config_key == "nfeatures":
                    orb_params["nfeatures"] = current_value
                elif config_key == "scaleFactor":
                    orb_params["scaleFactor"] = current_value / 10.0
                elif config_key == "nlevels":
                    orb_params["nlevels"] = current_value
                elif config_key == "ransacThreshold":
                    orb_params["ransacThreshold"] = current_value / 10.0
                # Update label
        ...
        if "command_save_to_hd5f" in toggles:
            orb_params["command_save_to_hd5f"] = toggles[
    ...
    params = [
        ...
        (
            language_config.ORB_NFEATURES_LABEL,
            100,
            5000,
            100,
            "nfeatures",
            1,
            str,
            language_config.ORB_NFEATURES_DESCRIPTION,
        ),
        (
            language_config.ORB_SCALEFACTOR_LABEL,
            10,
            20,
            1,
            "scaleFactor",
            10,
            lambda v: f"{v/10:.1f}",
            language_config.ORB_SCALEFACTOR_DESCRIPTION,
        ),
        (
            language_config.ORB_NLEVELS_LABEL,
            1,
            8,
            1,
            "nlevels",
            1,
            str,
            language_config.ORB_NLEVELS_DESCRIPTION,
        ),
        (
    ...
    for label_key, min_v, max_v, step, config_key, mult, fmt, tip in params:
        ...
        if config_key == "nfeatures" and initial_float_value == 0:
            initial_float_value = 1500
        if config_key == "scaleFactor" and initial_float_value == 0:
            initial_float_value = 1.1
        if config_key == "nlevels" and initial_float_value == 0:
            initial_float_value = 5
        if config_key == "ransacThreshold" and initial_float_value == 0:
            initial_float_value = 5.0
    ...
    extra_params_layout_main = (
        QVBoxLayout()
    )  # Gunakan VBox untuk menumpuk tombol dan folder
    ...
    command_save_hd5f_initial = orb_config.get(
        "command_save_to_hd5f", True
    )  # Default True?
    ...
    def on_align_folder_changed(index):
        selected_text = align_folder_dropdown.itemText(
            index
        )  # Dapatkan teks dari index yg dipilih

        if selected_text == language_config.SEARCH_SAVE_ALIGN_IMAGE_TO_FOLDER:
            start_dir = (
                folder_data["selected_path"]
                if os.path.isdir(folder_data["selected_path"])
                else ""
            )
            ...
            if folder_path:
                folder_data["selected_path"] = os.path.normpath(
                    folder_path
                )  # Simpan path asli
    ...
    extra_params_layout_main.addLayout(
        folder_selection_layout
    )  # Tambahkan layout folder ke VBox
