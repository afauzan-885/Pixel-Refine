...
from ..parameter_alignment.ofb_parameter_settings import (
    PARAMETER_SCHEMA as OFB_PARAMETER_SCHEMA,
    load_ofb_config,
    save_ofb_config,
...
ALIGNMENT_PARAMETER_PROVIDERS = {
    ...
    "OFB": {
        "schema": OFB_PARAMETER_SCHEMA,
        "load": load_ofb_config,
...
# ═══════════════════════════════════════════════════════════════════════
# SIMILARITY PARAMETER PAGE
# ═══════════════════════════════════════════════════════════════════════


@live_update("refresh_responsive_layout", on_resize=True)
class SimilarityParameterPage(QWidget):
    """Similarity denoising parameter page.

    Pure algorithm parameter controls (Tile Size, Overlap, Motion Sensitivity,
    Noise Offset, Early Exit, Equalize Brightness).
    TCM modules and compute dispatch are resolved automatically by the active AOT backend.
    """
    ...
    def _setup_ui(self):
        ...
        # Quarantined / internal keys not shown in the primary parameter UI
        hidden_keys = {
    ...
    def _reset_to_defaults(self):
        ...
        defaults["similarity_backend"] = "gpu"
...
class AlignmentParameterPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        config = self.provider["load"]() if self.provider.get("load") else {}
        self._loading = True
        schema = self.provider.get("schema", [])
        for field in schema:
            key = field["key"]
            value = config.get(key, field.get("default"))
        ...
        if not schema:
            info_desc = QLabel(
                "Block Flow uses auto-tuned 3-layer hierarchical optical flow with noise-aware pre-denoising and median filtering."
            )
            info_desc.setAlignment(Qt.AlignmentFlag.AlignCenter)
            info_desc.setWordWrap(True)
            info_desc.setStyleSheet("color: #7F8C8D; font-size: 9.5pt; font-style: italic; padding: 12px;")
