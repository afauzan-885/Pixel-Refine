...
from PySide6.QtCore import Qt, QThread, Signal, QMutex, QWaitCondition, QFile, QSemaphore, QTimer
import cv2
import numpy as np
import rawpy
    ...
    def run(self):
        ...
        try:
            ...
            if pil_thumb:
                ...
                # Save to cache
                result_image.save(cache_path, "JPG", 85)
        ...
        try:
            ...
            if ext in SUPPORTED_FORMATS.get("jpg", []) + SUPPORTED_FORMATS.get("png", []) + SUPPORTED_FORMATS.get("tiff", []):
                with Image.open(self.image_path) as img:
                    ...
                    img_corrected.thumbnail(self.thumbnail_size, Image.Resampling.LANCZOS)
...
def display_thumbnail_in_layout(layout, q_image, image_path, display_size=(80, 80), animator=None):
    """
    ...
    scaled_pixmap = pixmap.scaledToHeight(display_size[1], Qt.TransformationMode.SmoothTransformation)
    ...
    if animator:
        fade_in(animator, layout, thumb_label)
