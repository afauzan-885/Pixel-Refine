...
class Zoomable(QGraphicsView):
    ...
    def apply_zoom_level(self, target_level: int):
        ...
        if target_level != 0:
            if not (self._min_zoom_level <= target_level <= self._max_zoom_level):
                target_level = max(
                    self._min_zoom_level, min(target_level, self._max_zoom_level)
                )
                if target_level == 0:
                    return
    ...
    def apply_state(
        self, target_level: int, relative_center: tuple[float, float] | None
    ):
    ...
    def zoom_to_fit(self, rect=None):
        ...
        if rect is None and self.scene():
            rect = self.scene().itemsBoundingRect()

        if rect and not rect.isEmpty():
            self.fitInView(rect, Qt.AspectRatioMode.KeepAspectRatio)
            # Update internal zoom level approx?
            # For now, just reset the internal counter to 0 relative to this fit,
            # OR we accept that fitInView changes the matrix directly.
            # Ideally we sync self._zoom_level roughly or reset it.
            self._zoom_level = 0
    ...
    def get_view_state(self):
        ...
        return {
            ...
            "relative_center": self._get_relative_center(),
            # If we want to persist exact transform matrix (better for exact restore):
    ...
    def set_view_state(self, state):
        ...
        elif "zoom_level" in state:
            self.apply_state(state["zoom_level"], state.get("relative_center"))
