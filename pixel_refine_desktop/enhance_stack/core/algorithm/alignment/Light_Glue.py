...
def find_cudnn_dlls():
    ...
    env_vars = ["CUDNN_PATH", "CUDA_PATH"] + [
        k for k in os.environ.keys() if k.startswith("CUDA_PATH_V")
    ]
    for var in env_vars:
    ...
    if dlls:
        ...
        if cudnn_only:
            cudnn_only.sort(
                key=lambda d: int(d.stem.split("cudnn64_")[-1]), reverse=True
            )
...
class LightGlueAlgorithm:
    ...
    def _create_inference_session(self):
        ...
        try:
            ...
            usable = [
                p for p in preferred if (p if isinstance(p, str) else p[0]) in available
            ]

            if not usable:
                raise RuntimeError(
                    "❌ Tidak ada provider ONNX Runtime yang tersedia di sistem ini."
                )

            if use_gpu and any(
                "DmlExecutionProvider" in (p if isinstance(p, str) else p[0])
                for p in usable
            ):
                print(
                    "⚡ Menggunakan DirectML untuk akselerasi GPU (NVIDIA / AMD / Intel)."
                )
            elif use_gpu and any(
                "CUDAExecutionProvider" in (p if isinstance(p, str) else p[0])
                for p in usable
            ):
            ...
            self.sess = ort.InferenceSession(
                self.PIPELINE_ONNX, sess_options=sess_options, providers=usable
            )
    ...
    def run_inference_threaded(self, input_data, callback=None):
        ...
        thread = threading.Thread(
            target=self._inference_worker, args=(input_data, callback), daemon=True
        )
        thread.start()
    ...
    def calculate_global_motion(self, base_image, target_image, stop_requested=None):
        ...
        if megapixels > 22.0:
            ...
            target_image = cv2.resize(
                target_image, (new_width, new_height), interpolation=cv2.INTER_AREA
            )
            GRID_SIZE = (4, 4)
        ...
        if tile_w == 0 or tile_h == 0:
            return None, None
        overlap_w_px, overlap_h_px = int(tile_w * OVERLAP_PERCENT), int(
            tile_h * OVERLAP_PERCENT
        )
        ...
        def prep_for_onnx(img):
            ...
            if noise_level > 200.0:
                d, sigma_color, sigma_space = get_adaptive_bilateral(
                    noise_level, 200.0, 700.0, 5, 9, 20, 75
                )
                enhanced_img = cv2.bilateralFilter(
                    enhanced_img, d, sigma_color, sigma_space
                )
        ...
        while processed_tiles < num_tiles:
            ...
            try:
                ...
                processed_tiles += 1
                r, c, imgL, imgR, scaleL, offsetL, scaleR, offsetR, x_start, y_start = (
                    item
                )
            ...
            idx0, idx1, scores = (
                matches[batch_mask, 1],
                matches[batch_mask, 2],
                mscores[batch_mask],
            )
