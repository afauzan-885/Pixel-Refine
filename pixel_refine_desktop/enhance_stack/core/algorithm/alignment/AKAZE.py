...
class AKAZEAlgorithm:
    ...
    def compute_features_block(
        ...
        roi_base_enhanced = enhanced_gray_base[
            roi_y_start:roi_y_end, roi_x_start:roi_x_end
        ]
        roi_target_enhanced = enhanced_gray_target[
            roi_y_start:roi_y_end, roi_x_start:roi_x_end
        ]

        kps_base, desc_base = akaze_instance.detectAndCompute(roi_base_enhanced, None)
        kps_target, desc_target = akaze_instance.detectAndCompute(
            roi_target_enhanced, None
        )
        ...
        kps_target_adjusted, final_desc_target = adjust_and_filter_kps(
            kps_target, desc_target
        )
        ...
        kps_base_adjusted, final_desc_base = select_top_k(
            kps_base_adjusted, final_desc_base, max_kps_per_block
        )
        kps_target_adjusted, final_desc_target = select_top_k(
            kps_target_adjusted, final_desc_target, max_kps_per_block
        )
    ...
    def calculate_global_motion(
        ...
        def filter_worker(job_q, result_q):
            ...
            while True:
                ...
                try:
                    ...
                    enhanced_gray = prepare_image(
                        image_data, grayscale=True, use_clahe=True
                    )
        ...
        filter_thread = threading.Thread(
            target=filter_worker, args=(job_queue, result_queue)
        )
        filter_thread.start()
        ...
        if megapixels > 22.0:
            target_mp = 18.0
            scale_factor = (
                target_mp / megapixels
            ) ** 0.5  # Akar kuadrat karena area = w * h
        ...
        if scale_factor < 1.0:
            ...
            enhanced_base_gray = cv2.resize(
                enhanced_base_gray, (new_width, new_height), interpolation=interpolation
            )
            enhanced_target_gray = cv2.resize(
        ...
        def process_block(i, j):
            ...
            return self.compute_features_block(
                ...
                w,
                h,
                max_kps_per_block=max_kps_per_block,
            )
        ...
        try:
            ...
            if (
                len(keypoints_base_all) != descriptors_base_all.shape[0]
                or len(keypoints_target_all) != descriptors_target_all.shape[0]
            ):
                print(
                    "CRITICAL: Mismatch between keypoints and descriptors after filtering."
                )
                return None, None
        except Exception as e:
        ...
        keypoints_base_all, descriptors_base_all = select_top_k(
            keypoints_base_all, descriptors_base_all, 500
        )
        keypoints_target_all, descriptors_target_all = select_top_k(
            keypoints_target_all, descriptors_target_all, 500
        )
        ...
        try:
            ...
            ratio_thresh = 0.75
            good_matches = (
                [m for m, n in matches if m.distance < ratio_thresh * n.distance]
                if all(len(mn) == 2 for mn in matches)
                else []
            )
            ...
            good_matches = sorted(good_matches, key=lambda m: m.distance)[
                : akaze_config.get("max_keypoints_used", 500)
            ]
            pts_base = np.float32(
                [keypoints_base_all[m.queryIdx].pt for m in good_matches]
            ).reshape(-1, 1, 2)
            pts_target = np.float32(
                [keypoints_target_all[m.trainIdx].pt for m in good_matches]
            ).reshape(-1, 1, 2)
...
def main(
    ...
    try:
        ...
        print(
            f"A critical error occurred during the main pipeline: {e}\n{traceback.format_exc()}"
        )
    finally:
        # --- Tahap 4: Cleanup ---
        if h5f:
