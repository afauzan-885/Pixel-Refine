...
def block_cost_fft(ref: np.ndarray, comp: np.ndarray) -> float:
    """
    FFT-based cost calculation for large tiles.
    Matching C++ block_cost_fft().

    Uses phase correlation for robust matching.

    Args:
        ref: Reference tile (2D)
        comp: Comparison tile (2D)

    Returns:
        Cost value (0 = perfect match, 1 = no match)
    """
    try:
        import cv2
    except ImportError:
        # Fallback to simple cost
        return float(np.mean(np.abs(ref - comp)))
    ...
    # Pad images
    ref_padded = np.zeros((optimal_rows, optimal_cols), dtype=np.float32)
    comp_padded = np.zeros((optimal_rows, optimal_cols), dtype=np.float32)

    ref_padded[:rows, :cols] = ref.astype(np.float32)
    comp_padded[:rows, :cols] = comp.astype(np.float32)

    # Forward DFT
    ref_dft = cv2.dft(ref_padded, flags=cv2.DFT_COMPLEX_OUTPUT)
    comp_dft = cv2.dft(comp_padded, flags=cv2.DFT_COMPLEX_OUTPUT)

    # Cross-power spectrum (Ref * conj(Comp))
    cross = cv2.mulSpectrums(ref_dft, comp_dft, 0, conjB=True)

    # Robust normalization (phase correlation)
    eps = 1e-5
    magnitude = np.sqrt(cross[:, :, 0] ** 2 + cross[:, :, 1] ** 2) + eps
    cross[:, :, 0] /= magnitude
    cross[:, :, 1] /= magnitude

    # Inverse DFT
    correlation = cv2.idft(cross, flags=cv2.DFT_REAL_OUTPUT | cv2.DFT_SCALE)

    # Find peak in valid region
    valid_roi = correlation[:rows, :cols]
    max_val = np.max(valid_roi)

    # Convert correlation to cost
    cost = 1.0 - float(max_val)
    return max(0.0, cost)
