...
def perform_image_alignment(
    ...
    **kwargs,
    ...
    # =================================================================
    # === BACKEND SMART FLOW (NanoFlowNet) - SEQUENTIAL TILING ===
    # =================================================================
    elif optical_flow_type == "smart_flow":
        print("Memulai alignment menggunakan backend Smart Flow (NanoFlowNet) @ 50% res...", flush=True)
        from pixel_refine_desktop.enhance_stack.core.algorithm.alignment.alignment_features.smart_flow import (
            SmartFlowProcessor,
        )

        # [STABILITY] Support persistent sessions for incremental fusion
        processor_provided = kwargs.get("smart_flow_processor")
        processor = processor_provided if processor_provided else SmartFlowProcessor()
        try:
            # [PROXY SYSTEM] Prepare reference in uint8
            ref_uint8 = np.clip(reference_image_float * 255.0, 0, 255).astype(np.uint8)
            h_orig, w_orig = ref_uint8.shape[:2]
            
            # --- Optimization: 50% Working Resolution ---
            scale = 0.5
            w_low, h_low = int(w_orig * scale), int(h_orig * scale)
            ref_low = cv2.resize(ref_uint8, (w_low, h_low), interpolation=cv2.INTER_AREA)
            ...
            for i in range(1, num_images):
                ...
                # [PROXY SYSTEM] Convert current frame
                if is_linear_mode:
                    curr_float = normalize_image(curr_frame, curr_frame.dtype)
                    curr_proxy = to_gamma_proxy(curr_float, scale=proxy_scale)
                    curr_frame_u8 = np.clip(curr_proxy * 255.0, 0, 255).astype(np.uint8)
                else:
                    if curr_frame.dtype != np.uint8:
                        c_norm = normalize_image(curr_frame, curr_frame.dtype)
                        curr_frame_u8 = np.clip(c_norm * 255.0, 0, 255).astype(np.uint8)
                    else:
                        curr_frame_u8 = curr_frame

                # Resize current to 50%
                curr_low = cv2.resize(curr_frame_u8, (w_low, h_low), interpolation=cv2.INTER_AREA)

                # Estimate flow at 50% resolution
                flow_low = processor.process_image(
                    ref_low, curr_low, overlap=0.10, stop_requested=stop_requested, return_flow=True
                )

                if flow_low is not None:
                    # Upgrade flow to 100% resolution
                    h_curr, w_curr = curr_frame.shape[:2]
                    
                    # 1. Resize the flow map to MATCH target frame size exactly
                    flow_full = cv2.resize(flow_low, (w_curr, h_curr), interpolation=cv2.INTER_LINEAR)
                    
                    # 2. Multiply motion vectors by scaling ratio (dynamic scaling)
                    # Use actual ratios to be more robust than fixed 2.0
                    flow_full[:, :, 0] *= (w_curr / w_low)
                    flow_full[:, :, 1] *= (h_curr / h_low)
                    
                    # High-Res Warping on original raw image
                    aligned = warp_image_opencv(curr_frame, flow_full)
                    images[i] = aligned
                    
                    if save_align_image:
                        save_aligned_image(aligned, i + index_offset, "SmartFlow")
                else:
                    print(f"Warning: Smart Flow alignment failed for image {i+1}")
                ...
                if update_progress:
                    ...
                    update_progress(
                        current_msg_progress,
                        f"Alignment gambar {processed_count}/{num_images - 1} (Smart Flow)...",
                    )
                gc.collect()

            print("✅ Alignment Smart Flow selesai.")
            return True

        except Exception as e:
            print(f"Error kritis selama Smart Flow: {e}", flush=True)
            traceback.print_exc()
            return False
        finally:
            if not processor_provided:
                print("[DEBUG] Alignment Core: Melakukan release_sessions()...", flush=True)
                processor.release_sessions()
                print("[DEBUG] Alignment Core: Selesai release_sessions().", flush=True)
            else:
                print("[DEBUG] Alignment Core: Menjaga sesi Smart Flow tetap terbuka (Persistent).", flush=True)
