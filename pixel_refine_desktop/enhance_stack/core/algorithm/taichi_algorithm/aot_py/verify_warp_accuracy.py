...
def verify_accuracy():
    print("\n" + "="*60)
    print(" TAICHI AOT WARP ACCURACY VERIFICATION (vs OPENCV)")
    print("="*60)
    ...
    flow_u = 10.0 * np.sin(y / 50.0) # Horizontal shift
    flow_v = 10.0 * np.cos(x / 50.0) # Vertical shift
    flow = np.stack([flow_u, flow_v], axis=-1).astype(np.float32)

    # --- TAICHI AOT EXECUTION ---
    print(f"Running Taichi AOT Warp (3-Channel)...")
    start_ti = time.perf_counter()
    # Note: Taichi AOT expects i32 for images and f32 for flow
    res_ti_gpu = taichi_aot.warp_image(img_3ch, flow, return_gpu=True)
    res_ti = res_ti_gpu.to_numpy()
    del res_ti_gpu
    ti_time = time.perf_counter() - start_ti
    print(f"Taichi AOT Time: {ti_time*1000:.2f} ms")

    # --- OPENCV REFERENCE ---
    print(f"Running OpenCV Remap (INTER_CUBIC)...")
    start_cv = time.perf_counter()
    # cv2.remap expects map_x and map_y
    map_x = x + flow_u
    map_y = y + flow_v
    # OpenCV remap for 16-bit needs uint16
    ...
    # Convert back to float for error math
    ...
    # Normalize error relative to 16-bit range (65535)
    ...
    if error_percent <= 2.0:
        print(f"[PASS] Accuracy is within limits (< 2%)")
    else:
        print(f"[FAIL] Accuracy error ({error_percent:.4f}%) exceeds 2% limit!")

    # Save debug results
    debug_dir = os.path.join(file_dir, "test_output")
    os.makedirs(debug_dir, exist_ok=True)
    # Normalize to 8-bit for visualization
    cv2.imwrite(os.path.join(debug_dir, "warp_taichi_vis.png"), (res_ti // 256).astype(np.uint8))
    cv2.imwrite(os.path.join(debug_dir, "warp_opencv_vis.png"), (res_cv // 256).astype(np.uint8))
    cv2.imwrite(os.path.join(debug_dir, "warp_diff_vis.png"), (diff / max_err * 255).astype(np.uint8) if max_err > 0 else diff.astype(np.uint8))
