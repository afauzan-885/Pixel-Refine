...
def compile_cast_tcm():
    ...
    @ti.kernel
    def cast_u8_to_f32(src: ti.types.ndarray(dtype=ti.u8, ndim=3), dst: ti.types.ndarray(dtype=ti.f32, ndim=3)):
        for I in ti.grouped(src):
            dst[I] = ti.cast(src[I], ti.f32) / 255.0

    @ti.kernel
    def cast_f32_to_u8(src: ti.types.ndarray(dtype=ti.f32, ndim=3), dst: ti.types.ndarray(dtype=ti.u8, ndim=3)):
        for I in ti.grouped(src):
