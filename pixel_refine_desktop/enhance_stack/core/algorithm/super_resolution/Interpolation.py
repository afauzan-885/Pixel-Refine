...
class InterpolationAlgorithm:
    ...
    def average_stack(
        ...
        try:
            ...
            num_channels = (
                ref_image.shape[2] if ref_image.ndim == 3 else 0
            )  # 0 untuk grayscale
        ...
        for i, current_image in enumerate(images):
            ...
            current_num_channels = (
                current_image.shape[2] if current_image.ndim == 3 else 0
            )
        ...
        if num_images_averaged > 0:
            ...
            try:
                ...
                max_val = (
                    1.0 if np.issubdtype(dtype, np.floating) else 255
                )  # Default fallback
            ...
            if (
                stop_requested
                and stop_requested()
                and num_images_averaged < num_images_in_list
            ):
                pass
            return final_image
        else:
...
def main(
    ...
    try:
        ...
        align_dir = os.path.join(
            "database", "align"
        )  # Definisikan path folder alignment
        ...
        if not output_name_base_safe:
            output_name_base_safe = "stack_result"
        output_path = os.path.join(
            output_folder_stack, f"{output_name_base_safe}_average.tif"
        )
        ...
        if use_hdf5:
            ...
            for batch_start in range(0, total_images, batch_size):
                ...
                batch_images = image_processor.load_images_from_paths(
                    batch_paths, stop_requested
                )
                if stop_requested and stop_requested():
                    break  # Cek setelah loading
        ...
        if stop_requested and stop_requested():
            ...
            if stop_requested and stop_requested():
                ...
                if save_success:
                    ...
                    error_msg = (
                        language_config.FAILED_TO_SAVE_IMAGE
                        + f": {os.path.basename(output_path)}"
                    )
                    print(error_msg)
                    if update_progress:
                        update_progress(100, error_msg)  # Tetap 100% tapi pesan error
            else:
                print(language_config.FAILED_IMAGE_ENHANCEMENT)
                if update_progress:
                    update_progress(
                        100, language_config.FAILED_IMAGE_ENHANCEMENT
                    )  # 100% tapi pesan gagal

        elif not (
            stop_requested and stop_requested()
        ):  # Jika tidak ada hasil batch DAN tidak dibatalkan
        ...
        if stop_requested and stop_requested():
            if update_progress and progress_bar:
                update_progress(
                    progress_bar.value(), "Proses dibatalkan."
