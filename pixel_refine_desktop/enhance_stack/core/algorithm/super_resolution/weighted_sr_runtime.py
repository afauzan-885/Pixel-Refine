# weighted_sr_runtime.py - Production runtime for Weighted Super Resolution
...
# Compiled graph K values (must match weighted_sr.py compile output)
COMPILED_KS = [2, 3, 4, 5, 8]


class WSRAOTEngine:
    """
    Production runtime for GPU-accelerated Weighted Super Resolution.

    Drop-in replacement for the JIT TaichiWSR class. Same public API:
        solver = WSRAOTEngine(lr_shape, hr_shape, num_frames, scale=2)
    ...
    def __init__(self, lr_shape, hr_shape, num_frames, scale=2,
        ...
        tcm_path = os.path.join(tcm_dir, "weighted_sr_vulkan.tcm")
