...
def mean_division_vec3_weight_kernel(
    ...
    """Normalize RGB sums using per-channel weight."""
    for i, j in ti.ndrange(h, w):
...
def mean_division_vec3_scalar_weight_kernel(
    ...
    """Normalize RGB sums using scalar 2D weight."""
    for i, j in ti.ndrange(h, w):
...
def compile_spatial_tcm(arch=None, suffix="vulkan"):
    ...
    # Combined Fine Analysis 4 Passes
    ...
    sym_post_exp = ti.graph.Arg(ti.graph.ArgKind.SCALAR, "exponent", dtype=ti.f32)
    sym_post_cut = ti.graph.Arg(ti.graph.ArgKind.SCALAR, "cutoff", dtype=ti.f32)
    sym_post_h = ti.graph.Arg(ti.graph.ArgKind.SCALAR, "h", dtype=ti.i32)
    sym_post_w = ti.graph.Arg(ti.graph.ArgKind.SCALAR, "w", dtype=ti.i32)
    g_post = ti.graph.GraphBuilder()
    g_post.dispatch(
    ...
    module.add_graph("postprocess_spatial_weight", g_post.compile())

    # 7. Mean Division Vec3 Weight Graph
    sym_mean_sum_img = ti.graph.Arg(ti.graph.ArgKind.NDARRAY, "sum_img", dtype=ti.f32, ndim=3)
    sym_mean_sum_weight = ti.graph.Arg(ti.graph.ArgKind.NDARRAY, "sum_weight", dtype=ti.f32, ndim=3)
    sym_mean_ref_img = ti.graph.Arg(ti.graph.ArgKind.NDARRAY, "ref_img", dtype=ti.f32, ndim=3)
    sym_mean_dst = ti.graph.Arg(ti.graph.ArgKind.NDARRAY, "dst", dtype=ti.f32, ndim=3)
    sym_mean_h = ti.graph.Arg(ti.graph.ArgKind.SCALAR, "h", dtype=ti.i32)
    sym_mean_w = ti.graph.Arg(ti.graph.ArgKind.SCALAR, "w", dtype=ti.i32)
    ...
    module.add_graph("mean_division_vec3_weight", g_mean.compile())

    # 8. Mean Division Vec3 Scalar Weight Graph
    sym_mean_scalar_weight = ti.graph.Arg(ti.graph.ArgKind.NDARRAY, "sum_weight", dtype=ti.f32, ndim=2)
    g_mean_scalar = ti.graph.GraphBuilder()
    g_mean_scalar.dispatch(
        mean_division_vec3_scalar_weight_kernel,
        sym_mean_sum_img,
        sym_mean_scalar_weight,
        sym_mean_ref_img,
        sym_mean_dst,
        sym_mean_h,
        sym_mean_w,
    )
    module.add_graph("mean_division_vec3_scalar_weight", g_mean_scalar.compile())
...
def accumulate_spatial_merging_taichi(
    ...
    **kwargs,
    ...
    """
    Accumulates a frame into the global sum using its processed weight map via common AOT.
    Supports both 1-channel (2D) and 3-channel (3D vec3) weight maps and accumulators.
    """
    import taichi_vision.taichi_aot as taichi_aot
    return taichi_aot.accumulate_weighted_frame(
...
def postprocess_spatial_weight_taichi(
    ...
    h, w = int(weight_map.shape[0]), int(weight_map.shape[1])
    mod.run(
