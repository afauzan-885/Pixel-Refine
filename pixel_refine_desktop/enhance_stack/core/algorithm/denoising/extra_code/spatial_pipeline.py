...
def process_in_gpu(
    ...
    merging_backend="cpp",  # Default: "cpp" (stabil), opsi: "taichi" (full GPU)
    **kwargs,
):
    """Pipeline GPU Alignment + Merging (Fleksibel C++/Taichi).

    Alignment: Selalu GPU Taichi.
    Merging:   Bisa C++ (lebih stabil/hemat RAM) atau Taichi (lebih cepat/full VRAM).
    """
    from pixel_refine_desktop.enhance_stack.core.algorithm.alignment.alignment_features.global_feature import (
        estimate_noise_in_python,
        normalize_image,
    ...
    # 2. SELECTION FOR MERGING
    if merging_backend == "cpp":
        # --- PATH A: C++ MERGING (Identik dengan CPU path tapi alignment di GPU) ---
        ref_for_weight_calc = reference_image_float
        if is_linear_mode:
            ref_for_weight_calc = to_gamma_proxy(reference_image_float, scale=proxy_scale)
        ...
        def weight_map_producer_cpp(task_queue, result_queue, images_list_ref):
            local_curr_work_res = np.empty((work_res_h, work_res_w, 1), dtype=np.float32)
            ...
            while True:
                try:
                    ...
                    if is_linear_mode:
                        curr_float = to_gamma_proxy(curr_float, scale=proxy_scale)
                    curr_gray_preprocessed, _ = preprocess_in_python(curr_float)
                    curr_work_gray = cv2.resize(curr_gray_preprocessed, (work_res_w, work_res_h), interpolation=cv2.INTER_LINEAR)
                    local_curr_work_res[:, :, 0] = curr_work_gray
                    weight_map_work_res.fill(0)
                    c_interface.call_generate_weight_map_jit(
                        weight_map_sum=weight_map_work_res,
        ...
        task_queue = queue.Queue()
        result_queue = queue.Queue(maxsize=final_num_workers * 2)
        threads = [threading.Thread(target=weight_map_producer_cpp, args=(task_queue, result_queue, images)) for _ in range(final_num_workers)]
        for t in threads: t.start()
        for i in range(num_images): task_queue.put(i)
        for _ in range(final_num_workers): task_queue.put(None)
            ...
            while finished_count < num_images:
                ...
                try:
                    ...
                    if weight_map_uint16 is not None:
                        ...
                        if image_orig is not None:
                            w_float = weight_map_uint16.astype(np.float32) * (1.0 / 65535.0)
                            cv2.resize(w_float, (ref_image_w, ref_image_h), dst=consumer_weight_full_buf, interpolation=cv2.INTER_LINEAR)
                            if "norm_img_buf" not in locals():
                                norm_img_buf = np.empty((ref_image_h, ref_image_w, ref_channels_buffer), dtype=np.float32)
                            normalize_image(image_orig, ref_dtype, out=norm_img_buf)
                            if norm_img_buf.shape[:2] != (ref_image_h, ref_image_w):
                                norm_img = cv2.resize(norm_img_buf, (ref_image_w, ref_image_h), interpolation=cv2.INTER_AREA)
                            else: norm_img = norm_img_buf
                            final_image_sum_full_res += (norm_img * consumer_weight_full_buf[:, :, np.newaxis])
                            weight_map_sum_full_res += consumer_weight_full_buf
                            images[image_index] = None
                            del image_orig, weight_map_uint16, w_float
                            if norm_img is not norm_img_buf: del norm_img
                            processed_frames += 1
                            if processed_frames % 1 == 0: gc.collect()
                    finished_count += 1
                    result_queue.task_done()
                    if update_progress:
                        prog = int(pass_merge_range[0] + (finished_count / num_images) * (pass_merge_range[1] - pass_merge_range[0]))
                        msg = language_config.IMAGE_PROCESS_IN_PROGRESS.format(images_processed_so_far + finished_count, total_overall_images) if use_overall_progress else f"Merging frames: {finished_count}/{num_images} (GPU+CPP)"
                        update_progress(prog, msg)
                except queue.Empty:
        ...
        # --- PATH B: TAICHI MERGING (Full GPU Path) ---
        ...
        ref_gray_preprocessed_gpu = preprocess.preprocess_in_python_gpu(ref_for_weight_calc, use_sharpen=False)
        ref_noise_sigma = estimate_noise_in_python(ref_gray_preprocessed_gpu.to_numpy())
        ref_work_res_pass2_gpu = bilinear_resize(ref_gray_preprocessed_gpu, work_res_h, work_res_w)
        ...
        def _run_gpu_merging_loop():
            ...
            _weight_work_gpu = create_taichi_ndarray(np.zeros((work_res_h, work_res_w), dtype=np.float32))
            ...
            try:
                for i, img_orig in enumerate(images):
                    ...
                    # img_orig di sini sudah ti_ndarray (dari alignment return_format="ti_ndarray")
                    curr_full_gpu = preprocess.normalize_image_gpu(img_orig, dtype=ref_dtype, buffer_provider="pool")
                    ...
                    curr_work_gray_gpu = preprocess.preprocess_pipeline_gpu(
                        curr_full_gpu, normalize=False, apply_gamma=is_linear_mode,
                        extract_green=True, target_size=(work_res_h, work_res_w),
                        scale=proxy_scale, buffer_provider="pool")

                    generate_spatial_weights_taichi(
                        current_image=curr_work_gray_gpu, reference_image=ref_work_res_pass2_gpu,
                        weight_map_sum=_weight_work_gpu, base_window=_base_window_gpu,
                        stability_map=None, row_starts=_rows_gpu, col_starts=_cols_gpu,
                        tile_h=tile_h, tile_w=tile_w, noise_sigma=ref_noise_sigma,
                        motion_sensitivity=motion_sensitivity, noise_offset_factor=noise_offset_factor,
                        equalize_brightness=False, buffer_provider="pool",
                        search_radius=kwargs.get("similarity_search_radius", 3))

                    release_taichi_ndarray(curr_work_gray_gpu)
                    accumulate_spatial_merging_taichi(
                        current_image_full=curr_full_gpu, weight_map_work=_weight_work_gpu,
                        final_image_sum=_sum_gpu, weight_map_sum_full=_weight_sum_full_gpu, buffer_provider="pool")

                    release_taichi_ndarray(curr_full_gpu)
                    if hasattr(img_orig, "to_numpy"): # Jika ti_ndarray
                    ...
                    if update_progress:
                        prog = int(pass_merge_range[0] + ((i + 1) / num_images) * (pass_merge_range[1] - pass_merge_range[0]))
                        msg = language_config.IMAGE_PROCESS_IN_PROGRESS.format(images_processed_so_far + i + 1, total_overall_images) if use_overall_progress else f"Spatial Merging: {i+1}/{num_images} (GPU Taichi)"
                        update_progress(prog, msg)
                    time.sleep(0.01)
                ...
                for buf in [_sum_gpu, _weight_sum_full_gpu, _base_window_gpu, _rows_gpu, _cols_gpu, _weight_work_gpu]:
                    if buf: release_taichi_ndarray(buf)
                clear_vram()

        worker = get_taichi_worker()
        worker.submit_and_wait(_run_gpu_merging_loop)
        return (processed_frames_spatial[0], final_image_sum_full_res, weight_map_sum_full_res, ref_noise_sigma)
