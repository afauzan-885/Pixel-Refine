...
class MedianAlgorithm:
    def __init__(
        self, db_path, hdf5_path="database/align/aligned_images.h5", max_workers=None
    ):
        ...
        if update_progress:
            ...
            if total_overall_images is not None and total_overall_images > 0:
                progress_start_percent = (
                    images_processed_so_far / total_overall_images
                ) * 100
                theoretical_end_percent = (
                    (images_processed_so_far + num_images_in_this_call)
                    / total_overall_images
                ) * 100
                progress_end_percent_for_this_call = min(
                    theoretical_end_percent, overall_progress_cap
                )
                progress_range_for_this_call = max(
                    0, progress_end_percent_for_this_call - progress_start_percent
                )

            def map_progress(internal_percent, internal_message="process"):
                overall_progress = int(
                    progress_start_percent
                    + (internal_percent / 100.0) * progress_range_for_this_call
                )
    ...
    def _compute_median_image(
        ...
        def process_block(row_start, col_start):
            ...
            row_end, col_end = min(row_start + block_size, H), min(
                col_start + block_size, W
            )
            ...
            if num_channels > 0:
                block_stack = np.stack(
                    [im[row_start:row_end, col_start:col_end, :] for im in images]
                )
            else:
                block_stack = np.stack(
                    [im[row_start:row_end, col_start:col_end] for im in images]
            ...
            h_win = (
                base_hanning
                if (actual_h, actual_w) == (block_size, block_size)
                else np.outer(np.hanning(actual_h), np.hanning(actual_w))
            )

            weighted_block = block_result * (
                h_win[..., np.newaxis] if num_channels > 0 else h_win
            )
        ...
        if use_multi_core:
            ...
            future_to_task = {
                executor.submit(process_block, r, c): (r, c) for r, c in tasks
            }
            ...
            try:
                ...
                if update_progress:
                    progress_percent = int(
                        (blocks_processed_count / total_blocks) * internal_progress_cap
                    )
        ...
        if executor:
            executor.shutdown()  # Pastikan executor ditutup dengan baik
        ...
        final_weight_sum = (
            weight_sum[..., np.newaxis] if num_channels > 0 else weight_sum
        )
        ...
        if np.issubdtype(dtype, np.integer):
            ...
            stacked_image_float = np.nan_to_num(
                stacked_image_float, nan=0.0, posinf=max_val, neginf=min_val
            )
...
def main(
    ...
    try:
        ...
        output_path = os.path.join(
            output_folder_stack, f"{output_name_safe}_median.tif"
        )
