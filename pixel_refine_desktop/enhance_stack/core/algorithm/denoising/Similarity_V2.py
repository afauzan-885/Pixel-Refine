...
class SimilarityAlgorithmV2:
    def __init__(self, db_path, hdf5_path="database/align/aligned_images.h5"):
        ...
        self.high_global_sigma_thresh = (
            0.28  # Sigma di atas ini dianggap "sangat ber-noise"
        )
    ...
    def _calculate_dynamic_max_multiplier(self, global_avg_sigma):
        ...
        clamped_sigma = max(
            self.low_global_sigma_thresh,
            min(global_avg_sigma, self.high_global_sigma_thresh),
        )
        ...
        if clamped_sigma <= self.low_global_sigma_thresh:
            ...
            factor = (clamped_sigma - self.low_global_sigma_thresh) / (
                self.high_global_sigma_thresh - self.low_global_sigma_thresh
            )
            dynamic_multiplier = self.high_max_multiplier + factor * (
                self.low_max_multiplier - self.high_max_multiplier
            )
    ...
    def similarity_mfnr(
        ...
        stop_requested=None,
        lib_path="pixel_refine_desktop/ui/data/similarity_motion_v2.dll",
        ...
        if not (val_tile_h > 0 and val_tile_w > 0):
            raise ValueError(
                "tile_size must be a tuple or list of two positive integers."
            )
        ...
        if channels_ref_check != channels_buffer_cpp:
            raise RuntimeError(
                language_config.COLOR_CHANNEL_DOES_NOT_MATCH
                + f" Expected {channels_buffer_cpp}, got {channels_ref_check} from normalized ref."
            )

        final_image_sum = np.ascontiguousarray(
            np.zeros((h_ref, w_ref, channels_buffer_cpp), dtype=np.float32)
        )
        weight_map_sum = np.ascontiguousarray(
            np.zeros((h_ref, w_ref), dtype=np.float32)
        )  # Weight map selalu 1 channel
        ...
        for i, current_image_orig in enumerate(images):
            ...
            if update_progress:
                ...
                if total_overall_images is not None and total_overall_images > 0:
                    ...
                    current_progress_val = int(
                        (overall_processed_count / total_overall_images)
                        * progress_cap_percent
                    )
                    progress_message = language_config.IMAGE_PROCESS_IN_PROGRESS.format(
                        overall_processed_count, total_overall_images
                    )
                else:
                    current_progress_val = int(
                        ((i + 1) / num_images_total) * progress_cap_percent
                    )
                    progress_message = language_config.ANALYZING_IMAGE.format(
                        i + 1, num_images_total
                    )
            ...
            try:
                if (
                    current_image_orig.shape[0] != h_orig
                    or current_image_orig.shape[1] != w_orig
                ):
                    continue
        ...
        if processed_frames_count == 0:
            if channels_orig == 1 and ref_image_orig.ndim == 2:  # input asli grayscale
                ...
                output_shape = (
                    (h_orig, w_orig)
                    if channels_orig == 1
                    else (h_orig, w_orig, channels_orig)
                )
                return np.zeros(output_shape, dtype=dtype_orig)

        try:
            c_interface.normalize_accumulated(
                final_image_sum, weight_map_sum, h_ref, w_ref, channels_buffer_cpp
            )
        except Exception as e:
        ...
        if save_weight_map_path:
            ...
            try:
                if processed_frames_count > 0:
                    normalized_weights_vis = weight_map_sum / float(
                ...
                if success_save:
                    ...
                    print(
                        language_config.FAILED_TO_SAVE_WEIGHT_MAP_TO_PATH.format(
        ...
        if channels_orig == 1:
            ...
            final_image_output = np.clip(
                final_image_gray_float, 0, np.iinfo(dtype_orig).max
            ).astype(dtype_orig, copy=False)
        elif channels_orig == 3:
            final_image_output = np.clip(
                final_image_scaled, 0, np.iinfo(dtype_orig).max
            ).astype(dtype_orig, copy=False)
        ...
        if (
            stop_requested
            and stop_requested()
            and processed_frames_count < num_images_total
        ):
            pass
...
def main(
    ...
    try:
        ...
        sim_overlap_percent = general_settings.get(
            "similarity_v2_overlap_percent", 40.0
        )
        sim_v2_mbm_noise_mad_offset_factor = general_settings.get(
            "similarity_v2_mbm_noise_mad_offset_factor", 0.5
        )
        sim_v2_mbm_mad_sensitivity = general_settings.get(
            "similarity_v2_mbm_mad_sensitivity", 20.0
        )
        sim_v2_mbm_confidence_skip_dft_threshold = general_settings.get(
            "similarity_v2_mbm_confidence_skip_dft_threshold", 0.9
        )
        sim_v2_freq_merge_wiener_c_factor = general_settings.get(
            "similarity_v2_freq_merge_wiener_c_factor", 2.0
        )
        sim_v2_coarse_alignment_search_margin = general_settings.get(
        ...
        if single_process:
            ...
            if image_paths:
                if image_paths[0] and isinstance(image_paths[0], str):
                    ref_image_name = os.path.splitext(os.path.basename(image_paths[0]))[
                        0
                    ]
        ...
        output_name_base_safe = "".join(
            c for c in output_name_base if c.isalnum() or c in ("_", "-")
        ).rstrip()
        if not output_name_base_safe:
            output_name_base_safe = "stack_result"  # Fallback name
        output_path = os.path.join(
            output_folder_stack, f"{output_name_base_safe}_similarityV2.tif"
        )
        weight_map_output_path = os.path.join(
            output_folder_stack, f"{output_name_base_safe}_similarity_weight_map.png"
        )
        ...
        if use_hdf5:
            ...
            try:
                ...
                print(
                    language_config.NUMBER_OF_IMAGES_TO_BE_PROCESSED.format(
                        total_images
                    )
                )
                ...
                if total_images == 0:
                    if update_progress:
                        update_progress(100, "File HDF5 is empty.")
                    return

                total_batches = (total_images + batch_size - 1) // batch_size
                print(
                    language_config.NUMBER_OF_BATCHES_TO_BE_PROCESSED.format(
                        total_batches
                    )
                )
                ...
                with h5py.File(global_hdf5_path, "r") as h5f:
                    ...
                    for batch_start in range(0, total_images, batch_size):
                        ...
                        print(
                            f"\n"
                            + language_config.PROCESSING_BATCH.format(
                                current_batch_num, total_batches, batch_start
                            )
                        )
                        ...
                        batch_keys = keys[
                            batch_start : min(batch_start + batch_size, total_images)
                        ]
                        print(
                            language_config.LOAD_IMAGE_FROM_HDF5.format(len(batch_keys))
                        )
                        batch_images = []
                        ...
                        for key in batch_keys:
                            ...
                            try:
                                ...
                                print(
                                    language_config.ERROR_WHILE_RETRIEVING_KEY_FROM_HD5F.format(
                                        key, e
                                    )
                                )
                        if stop_requested and stop_requested():
                            break  # Cek lagi setelah loop key
                        ...
                        if not batch_images:
                            ...
                            continue
                        batch_images, new_size = resize_all_with_padding(
                            batch_images, method="preserve"
                        )
                        print(
                            language_config.START_IMAGE_ENHANCEMENT.format(
                                len(batch_images)
                            )
                        )
                        try:
                        ...
                        if batch_result is not None:
                            ...
                            print(
                                f"Batch {current_batch_num} processing failed or returned None."
                            )
                ...
                if update_progress:
                    update_progress(
                        0, language_config.ERROR_IN_READING_FILE_HDF5.format(e)
                    )
                return
            ...
            if total_images == 0:
                print(
                    language_config.NO_IMAGE_PATH_PROCESSED_IMAGE
                )  # Redundan, sudah dicek di atas, tapi aman
                if update_progress:
                    update_progress(100, language_config.NO_IMAGE_PATH_PROCESSED_IMAGE)
                return
            ...
            total_batches = (total_images + batch_size - 1) // batch_size
            print(
                language_config.NUMBER_OF_BATCHES_TO_BE_PROCESSED.format(total_batches)
            )
        ...
        if stop_requested and stop_requested():
            ...
            final_weight_map_path_arg = (
                weight_map_output_path if save_final_weight_map else None
            )
            if final_weight_map_path_arg:
                print(
                    f"Final weight map saving is ENABLED to: {final_weight_map_path_arg}"
                )
            else:
