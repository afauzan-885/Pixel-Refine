...
class SpatialFusionDenoisingAlgorithm:
    ...
    def run(self, ctx, frames, batch_plan=None):
        ...
        kwargs = dict(
            ...
            # Internal alignment remains the native backend-qualified
            # compute_flow_<arch>.tcm path.  It is independent from the
            # external MFDenoiser alignment selection.
        ...
        # The processor reserves the device while alignment and merging run.
        # Force its AOT calls onto the caller thread so the alignment TCM load
        # cannot deadlock behind that reservation in TaichiWorker.
