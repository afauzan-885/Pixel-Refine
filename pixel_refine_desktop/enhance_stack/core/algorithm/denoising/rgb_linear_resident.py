...
class BlockMatchingGPUResidentAligner:
    ...
    def estimate_flow(self, analysis_frame_gpu, *, stop_event=None):
        """Return an owned block-matching flow field without warping RGB."""
        from taichi_vision import taichi_aot
        ...
        analysis_gpu = analysis_frame_gpu
        owned_analysis = False
        analysis_gray_gpu = None
        flow_gpu = None
        try:
            ...
            raise RuntimeError(
                "Native Block Matching GPU estimation failed; no CPU fallback is "
                f"permitted in the resident pipeline: {exc}"
            ) from exc
        finally:
            if flow_gpu is not None and hasattr(flow_gpu, "release"):
    ...
    def estimate_alignment(self, analysis_frame_gpu, *, stop_event=None):
        """Expose Block Matching through the shared estimator contract."""
        from .resident_alignment import ResidentAlignmentEstimate

        flow_gpu = self.estimate_flow(analysis_frame_gpu, stop_event=stop_event)
        return ResidentAlignmentEstimate(
            plan="block_matching",
            full_shape=(self.full_h, self.full_w),
            proxy_shape=self.ref_shape,
            flow_gpu=flow_gpu,
        )
...
class TaichiDenseFlowResidentAligner:
    ...
    def estimate_flow(self, analysis_frame_gpu, *, stop_event=None):
        """Return an owned RGB-proxy flow field without warping pixel data."""
        from taichi_vision import taichi_aot
        ...
        try:
            ...
            raise RuntimeError(
                f"Native Taichi {self.flow_type} estimation failed; "
    ...
    def estimate_alignment(self, analysis_frame_gpu, *, stop_event=None):
        """Expose dense flow through the shared estimator contract."""
        from .resident_alignment import ResidentAlignmentEstimate
        ...
        return ResidentAlignmentEstimate(
            plan=self.flow_type,
            full_shape=(self.full_h, self.full_w),
            proxy_shape=(self.ref_h, self.ref_w),
            flow_gpu=flow_gpu,
        )
    ...
    def align_frame(
        ...
        *,
        ...
        analysis_gpu = analysis_frame_gpu if analysis_frame_gpu is not None else supp_linear_gpu
        flow_gpu = None
        warped_secondary = None
        try:
            flow_gpu = self.estimate_flow(analysis_gpu, stop_event=stop_event)
...
class FeatureMatchingGPUAligner:
    ...
    def estimate_homography(self, analysis_frame_gpu, *, stop_event=None):
        """Return support-to-reference geometry without warping RGB pixels.

        RAW Native uses this public estimator boundary to reuse the exact OFB
        or AKAZE feature/RANSAC implementation while applying the resulting
        transform later to independent Bayer planes.
        """
        from taichi_vision import taichi_aot
        ...
        supp_gray_gpu = taichi_aot.cvtColor(
            analysis_frame_gpu, taichi_aot.COLOR_RGB2GRAY
        )
        ...
        return np.ascontiguousarray(
            scale_to_full @ H_work @ scale_to_work, dtype=np.float32
        )

    def estimate_alignment(self, analysis_frame_gpu, *, stop_event=None):
        """Expose OFB/AKAZE geometry through the shared estimator contract."""
        from .resident_alignment import ResidentAlignmentEstimate

        matrix = self.estimate_homography(analysis_frame_gpu, stop_event=stop_event)
        if matrix is None:
            return None
        return ResidentAlignmentEstimate(
            plan=self.matcher_name,
            full_shape=(self.target_h, self.target_w),
            proxy_shape=(self.work_h, self.work_w),
            homography=matrix,
        )
    ...
    def align_frame(
        ...
        *,
        ...
        src_for_analysis = analysis_frame_gpu if analysis_frame_gpu is not None else supp_linear_gpu
        H_full = self.estimate_homography(src_for_analysis, stop_event=stop_event)
        ...
        if H_full is not None:
            ...
            if secondary_frame_to_warp is not None:
                warped_secondary = taichi_aot.warp_perspective(
                    secondary_frame_to_warp,
                    # Secondary analysis buffers are still in the work domain.
                    np.array(
                        [
                            [self.work_w / self.target_w, 0, 0],
                            [0, self.work_h / self.target_h, 0],
                            [0, 0, 1.0],
                        ],
                        dtype=np.float32,
                    ) @ H_full @ np.array(
                        [
                            [self.target_w / self.work_w, 0, 0],
                            [0, self.target_h / self.work_h, 0],
                            [0, 0, 1.0],
                        ],
                        dtype=np.float32,
                    ),
...
class RGBLinearResidentProcessor:
    """Established RGB-float32 implementation behind the resident router."""

    process_format = "RGB Linear"

    def run(self, image_paths, session=None, **kwargs):
        return run_gpu_resident_pipeline(image_paths, session, **kwargs)


# Canonical alias for direct RGB-processor callers.
