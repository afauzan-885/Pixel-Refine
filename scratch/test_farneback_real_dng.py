# test_farneback_real_dng.py - GPU Farneback V2 vs OpenCV on Real DNG Images
...
def resize_with_aspect(img, max_size=MAX_IMAGE_SIZE):
...
def estimate_shift(flow, h, w, negate=False, scale=1.0):
    """
    Estimate mean (dx, dy) from center crop of flow field.
    GPU flow convention: -(cmp - ref) * 0.5 (inverted and scaled by 0.5)
    OpenCV convention: (cmp - ref)
    
    To convert GPU flow to OpenCV convention:
    1. Negate (to get (cmp - ref) * 0.5)
    2. Scale by 2.0 (to get (cmp - ref))
    ...
    return sign * scale * np.mean(crop[..., 0]), sign * scale * np.mean(crop[..., 1])
...
def run_real_dng_test(engine, mod, pyramid_mod, ref_path, comp_path, demosaic_algo,
    ...
    # GPU flow convention: -(cmp - ref) * 0.5 (inverted relative to OpenCV)
    # OpenCV convention: (cmp - ref)
    # To convert: negate (to get (cmp - ref) * 0.5)
    # Note: The 0.5 factor is part of gradient averaging, not flow scaling
    # Based on debug output, GPU flow is inverted in both x and y
    # So we need to negate the entire flow
    gpu_est_dx, gpu_est_dy = estimate_shift(gpu_flow, h, w, negate=True, scale=1.0)
    ...
    # Convert GPU flow to OpenCV convention for warping
    # GPU flow convention: -(cmp - ref) * 0.5 (inverted relative to OpenCV)
    # OpenCV convention: (cmp - ref)
    # Based on debug output, GPU flow is inverted in both x and y
    # So we need to negate the entire flow
    gpu_flow_cv = -gpu_flow
    
    # --- Warp comparison images for SSIM ---
    warped_cv = warp_flow_opencv(comp_u8, cv_flow)
    warped_gpu = warp_flow_opencv(comp_u8, gpu_flow_cv)
    ...
    flow_magnitude_gpu = np.sqrt(gpu_flow_cv[..., 0]**2 + gpu_flow_cv[..., 1]**2)
...
def main():
    print("=" * 120)
    print("GPU FARNEBACK V2 REAL DNG IMAGE TEST")
    print("=" * 120)
    ...
    results_file = os.path.join(test_algorithm_dir, "farneback_real_dng_results.txt")
    with open(results_file, 'w') as f:
        f.write("GPU Farneback V2 Real DNG Image Test Results\n")
        f.write("=" * 60 + "\n\n")
        for r in results:
