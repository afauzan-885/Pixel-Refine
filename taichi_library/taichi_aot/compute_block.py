...
from .generic_block import BlockComputeSpec, run_generic_blocks
...
class _SourcePatternVisitor(ast.NodeVisitor):
    ...
    def visit_Subscript(self, node: ast.Subscript) -> Any:  # noqa: N802
        ...
        parts = list(slice_node.elts) if isinstance(slice_node, ast.Tuple) else [slice_node]
    ...
    def walk(value: Any, setter: Callable[[Any], None], path: tuple[Any, ...]):
        ...
        if isinstance(value, tuple):
            ...
            for index, item in enumerate(value):
                yield from walk(
                    item,
                    lambda replacement, value=value, index=index: value.__setitem__(index, replacement),
                    path + (index,),
                )
            ...
            for key, item in value.items():
                yield from walk(
                    item,
                    lambda replacement, value=value, key=key: value.__setitem__(key, replacement),
                    path + (key,),
                )
...
def _replace_value(value: Any, replacements: Mapping[int, Any], block) -> Any:
    ...
    if isinstance(value, dict):
        return {key: _replace_value(item, replacements, block) for key, item in value.items()}
    return value
...
def _replace_call(args, kwargs, replacements, block):
    return (
        ...
        {key: _replace_value(value, replacements, block) for key, value in kwargs.items()},
    )
...
class _ComputeBlockDirective:
    ...
    @staticmethod
    def _should_bypass(metadata: ComputeBlockMetadata, args, kwargs) -> bool:
        if os.environ.get("PIXEL_REFINE_AUTO_BLOCK", "1").strip().lower() in {
            "0",
            "false",
            "off",
        }:
            return True
        if metadata.mode == "off" or not metadata.automatic:
            return True
        if kwargs.get("return_gpu") is True:
            return True
        if any(key in kwargs and kwargs[key] is not None for key in ("dst", "out", "output")):
            return True
        if metadata.analysis.existing_tile_loop:
    ...
    @staticmethod
    def _collect_arrays(args, kwargs):
        entries = []
        seen = set()
        ...
        matching = [item for item in entries if tuple(item[1].shape[:2]) == shape]
        if not matching:
            return None, None, "no matching image shapes"
        return matching, shape, ""

    def _wrap_function(self, function: Callable[..., Any], metadata: ComputeBlockMetadata):
        @functools.wraps(function)
        def wrapped(*args, **kwargs):
            depth = _DISPATCH_DEPTH.get()
            if depth:
                return function(*args, **kwargs)
            if self._should_bypass(metadata, args, kwargs):
                return function(*args, **kwargs)

            entries, shape, reason = self._collect_arrays(args, kwargs)
            if entries is None:
                return function(*args, **kwargs)
            ...
            def invoke_tile(context):
                tile_args, tile_kwargs = _replace_call(args, kwargs, replacements, context.block)
            ...
            try:
                ...
                # ``return_none`` is an explicit policy; the default
                # full-frame fallback already returns the reference value.
