"""
compile_template_flow_tcm.py — AOT Compilation for Horn-Schunck Optical Flow
=============================================================================
...
from taichi_library.taichi_algorithm import template_flow as tf
...
def compile_horn_schunck_flow(arch=ti.vulkan, out_dir=None):
    ...
    Big Graph: hs_align_3layer_{N}
        5-step pipeline:
          ...
          2. _upsample_flow_bicubic_kernel (L2 → L1)
          3. _hs_refinement_level_kernel (gradient + project + N Jacobi)
          4. _upsample_flow_bicubic_kernel (L1 → L0)
    ...
    for N in (10, 20, 50):
        ...
        # Step 1: Coarsest level (L2) — gradient + zero-init + N Jacobi
        g.dispatch(
        ...
        # Step 3: Refinement at L1 — gradient + project + N Jacobi
        g.dispatch(
        ...
        # Step 5: Refinement at L0 — gradient + project + N Jacobi
        g.dispatch(
            tf._hs_refinement_level_kernel,
    ...
    tcm_name = f"template_flow_{arch_name}.tcm"
...
if __name__ == "__main__":
    ...
    compile_horn_schunck_flow(arch=ti.vulkan)
