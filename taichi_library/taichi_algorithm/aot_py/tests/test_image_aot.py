...
from taichi_library.taichi_algorithm.compression.jpeg_aot import encode_grayscale_aot, encode_rgb_aot
...
def _morph_reference(src: np.ndarray, op: str) -> np.ndarray:
    ...
    for y in range(src.shape[0]):
        for x in range(src.shape[1]):
            values = [
                src[_reflect101(y + dy, src.shape[0]), _reflect101(x + dx, src.shape[1])]
                for dy in (-1, 0, 1)
                for dx in (-1, 0, 1)
            ]
...
def run_accuracy_suite() -> dict[str, float]:
    ...
    errors["dilate"] = float(np.max(np.abs(dilate_aot(source, ksize=3) - _morph_reference(source, "dilate"))))
    errors["erode"] = float(np.max(np.abs(erode_aot(source, ksize=3) - _morph_reference(source, "erode"))))
    ...
    errors["warp_identity"] = float(np.max(np.abs(warp_affine_aot(source, identity, (6, 6)) - source)))
    ...
    rgb_border = copy_make_border_aot(np.repeat(source[..., None], 3, axis=2), 1, 1, 1, 1, "CONSTANT", value=(3.0, 5.0, 7.0))
    if not (np.all(rgb_border[0, :, 0] == 3.0) and np.all(rgb_border[0, :, 1] == 5.0) and np.all(rgb_border[0, :, 2] == 7.0)):
        raise AssertionError("per-channel constant border mismatch")
    errors["border"] = 0.0

    normalized = normalize_aot(source, 0.0, 1.0)
    errors["normalize"] = max(abs(float(normalized.min())), abs(float(normalized.max()) - 1.0))
    ...
    errors["gaussian_window"] = 0.0 if np.all(np.isfinite(window)) and np.all(window > 0.0) else 1.0
