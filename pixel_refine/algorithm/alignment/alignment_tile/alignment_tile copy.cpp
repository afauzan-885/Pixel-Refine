...
struct Candidate {
    ...
    // Konstruktor default (opsional, tapi bagus untuk dimiliki)
    Candidate() : flow(0,0), cost(0.0f) {}

    // KONSTRUKTOR YANG HILANG:
    // Memberi tahu C++ cara membuat Candidate dari Point2f dan float.
    Candidate(const cv::Point2f& f, float c) : flow(f), cost(c) {}
};
...
namespace OpticalFlowHelpers
{
     ...
     */
    static inline void searchFineLevel(
        std::vector<Candidate>& out_candidates, // <-- Parameter output untuk diisi
        const cv::Mat& ref_layer,
        const cv::Mat& comp_layer,
        int tile_y, int tile_x,
        int tile_h, int tile_w,
        const cv::Vec2f& initial_flow)
    {
        // 1. Bersihkan buffer dari iterasi sebelumnya, TIDAK mengalokasi ulang memori.
        ...
        if (test_y < 0 || test_x < 0 ||
            test_y + tile_h > h_layer ||
            test_x + tile_w > w_layer)
            return; // Kembali dengan buffer kosong
        ...
        // 2. Isi buffer output. 'emplace_back' lebih disukai jika Anda punya konstruktor.
        // Jika tidak, push_back({...}) juga bagus.
        out_candidates.push_back({cv::Point2f(init_dx, init_dy), current_cost * tile_area_inv});
        // (Asumsi Anda sudah menambahkan konstruktor ke struct Candidate)
    }
    ...
    static cv::Point2f selectBestCandidate(
        ...
        int tile_h, int tile_w)
    {
        // ... (Langkah 1: Penanganan Kasus Tepi) ...
        if (candidates.empty()) {
...
#pragma omp parallel
        {
            ...
            std::vector<Candidate> fine_level_buffer;
            fine_level_buffer.reserve(1); // Hanya butuh ruang untuk 1 kandidat
