...
class DatabaseManager:
        ...
        try:
            with self._get_connection() as conn:
                ...
                if not cursor.fetchone():
                    ...
                    """)
                    print("Table 'batch_process_image' created with 'is_reference_batch' column.")
                else:
            ...
            with self._get_connection() as conn:
                ...
                for image_path in image_paths:
                    try:
                        ...
                        cursor.execute(sql_insert_link, (batch_id, image_id, current_is_reference))
                        added_count += 1
    ...
    def set_batch_process_reference(self, batch_id, image_path):
        ...
        sql_reset_batch_refs = "UPDATE batch_process_image SET is_reference_batch = 0 WHERE batch_id = ?"
        ...
        try:
            with self._get_connection() as conn:
                ...
                try:
                    ...
                    if not img_result:
                        print(f"Error setting batch reference: Image path '{image_path}' not found in 'images' table.")
                        return False
                    ...
                    if not cursor.fetchone():
                        print(f"Error setting batch reference: Image ID {image_id} ('{image_path}') is not part of batch ID {batch_id}.")
                    ...
                    if cursor.rowcount == 0:
                        ...
                        print(f"Error setting batch reference: Failed to update reference status for image ID {image_id} in batch ID {batch_id}.")
                        conn.rollback()
                        return False

                    conn.commit()
                    print(f"Image '{image_path}' (ID: {image_id}) is now the reference for batch ID {batch_id}.")
                    return True

                except sqlite3.Error as e:
                    print(f"Database error setting batch reference for batch {batch_id}, image '{image_path}': {e}")
                    conn.rollback()
                    return False
        except sqlite3.Error as e:
    ...
    def batch_process_delete_image(self, batch_id, image_id): # Perlu dipertimbangkan jika ref dihapus
        """
        ...
        sql_delete = "DELETE FROM batch_process_image WHERE batch_id = ? AND image_id_batch = ?"
        ...
        try:
            with self._get_connection() as conn:
                ...
                if deleted_row_count > 0:
                    print(f"Image link (Image ID: {image_id}) deleted from batch ID: {batch_id}.")
                    # 3. Jika referensi dihapus dan masih ada gambar lain di batch ini, set referensi baru
                    if was_ref:
                        cursor.execute("SELECT COUNT(*) FROM batch_process_image WHERE batch_id = ?", (batch_id,))
                        remaining_count = cursor.fetchone()[0]
                        if remaining_count > 0:
                            cursor.execute(sql_find_new_ref, (batch_id,))
                            new_ref_bpi_id_result = cursor.fetchone() # ini adalah id dari batch_process_image, bukan image_id
                            if new_ref_bpi_id_result:
                                new_ref_bpi_id = new_ref_bpi_id_result[0]
                                cursor.execute(sql_set_ref, (new_ref_bpi_id,))
                                print(f"Previous reference in batch {batch_id} deleted. New reference set for bpi.id: {new_ref_bpi_id}.")
                    conn.commit()
        except sqlite3.Error as e:
    ...
    def batch_process_delete_selected_images(self, batch_id, image_paths_to_delete):
        ...
        sql_find_new_ref = "SELECT id FROM batch_process_image WHERE batch_id = ? ORDER BY id LIMIT 1"
        sql_set_ref = "UPDATE batch_process_image SET is_reference_batch = 1 WHERE id = ?"
            ...
            with self._get_connection() as conn:
                ...
                try:
                    ...
                    if not images_info_to_delete:
                        print(f"No images from the provided list found in batch ID {batch_id}.")
                        return 0
                    ...
                    if was_any_ref_deleted:
                        ...
                        remaining_count = cursor.fetchone()[0]
                        if remaining_count > 0:
                            cursor.execute("UPDATE batch_process_image SET is_reference_batch = 0 WHERE batch_id = ?", (batch_id,))
                            cursor.execute(sql_find_new_ref, (batch_id,))
                            new_ref_result = cursor.fetchone()
                            if new_ref_result:
                                new_ref_bpi_id = new_ref_result[0]
                                cursor.execute(sql_set_ref, (new_ref_bpi_id,))
                                print(f"A reference image was deleted from batch {batch_id}. New reference set for bpi.id: {new_ref_bpi_id}.")

                    conn.commit()
                    if deleted_count > 0 :
                        print(f"Successfully removed {deleted_count} image(s) from batch ID {batch_id}.")
                    else:
                        print(f"No images were removed from batch ID {batch_id} (either not found in batch or already removed).")
                    return deleted_count

                except sqlite3.Error as e:
                    print(f"Database error during selected image deletion from batch {batch_id}: {e}")
                    conn.rollback()
                    return -1
        except sqlite3.Error as e:
    ...
    def single_process_delete_path_images(self, image_paths):
        ...
        sql_find_new_ref = "SELECT image_id_single FROM single_process_image ORDER BY id LIMIT 1"
