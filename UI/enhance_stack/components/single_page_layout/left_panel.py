...
class LeftPanel(QWidget):
    ...
    def init_parameter_panel(self, parent_layout):
        ...
        self.alignment_dropdown, alignment_widget = self.create_dropdown(alignment_display_name, alignment_names, alignment_descs)
        ...
        self.super_resolution_dropdown, super_resolution_widget = self.create_dropdown(super_res_display_name, super_res_names, super_res_descs)
        ...
        self.denoising_dropdown, denoising_widget = self.create_dropdown(denoising_display_name, denoising_names, denoising_descs)
        ...
        for btn, source_key in self.parameter_switch_buttons_map.items():
            btn.setStyleSheet(SWITCH_BUTTON_DEFAULT_STYLE) # Terapkan style default awal
        ...
        self.alignment_dropdown.currentIndexChanged.connect(lambda: self.handle_dropdown_change_for_source("alignment"))
        self.super_resolution_dropdown.currentIndexChanged.connect(lambda: self.handle_dropdown_change_for_source("super_resolution"))
        self.denoising_dropdown.currentIndexChanged.connect(lambda: self.handle_dropdown_change_for_source("denoising"))
    ...
    def create_dropdown(self, label_text, items, tooltips):
        ...
        label.setStyleSheet("font-weight: bold; margin-bottom: 20x;")
        dropdown = QComboBox()
    ...
    def handle_dropdown_change_for_source(self, source_category):
        ...
        if source_category == "denoising" and self.denoising_dropdown.currentIndex() != 0:
        ...
        elif source_category == "super_resolution" and self.super_resolution_dropdown.currentIndex() != 0:
        ...
        if target_button:
            if not target_button.isChecked():
                target_button.setChecked(True) # Ini akan memanggil on_switch_button_clicked
            else:
