...
class OrbAlignment(QThread):
    progress_update = pyqtSignal(int, str)
    finished = pyqtSignal()
    error_signal = pyqtSignal(str)

    def __init__(self, db_path, debug_folder="database/align/global/debug_images", hdf5_path="database/align/aligned_images.h5"):
        super().__init__()
    ...
    def load_images_from_paths(self, image_paths):
        """
        Memuat gambar dengan format 16-bit dari daftar path gambar.
        """
        images = []
        for image_path in image_paths:
            # Membaca gambar dengan format yang tidak dimodifikasi, termasuk 16-bit
            image = cv2.imread(image_path, cv2.IMREAD_UNCHANGED)
            if image is not None:
                images.append(image)
            else:
                print(language_config.LOAD_IMAGES_FROM_PATHS_LOAD_FAILED.format(image_path))
        return images


    def align_images(self, base_image, target_image, image_id):
        print(language_config.ALIGN_IMAGES_STATUS_ORB.format(image_id=image_id))

        kp1, des1 = self.orb.detectAndCompute(base_image, None)
        kp2, des2 = self.orb.detectAndCompute(target_image, None)

        if des1 is None or des2 is None or len(kp1) < 2 or len(kp2) < 2 : #penanganan jika tidak ada fitur yang terdeteksi
    ...
    def save_to_hdf5(self, aligned_images):
        print(language_config.SAVE_TO_HDF5_ALIGNED_SAVING.format(self=self))
        with h5py.File(self.hdf5_path, "w") as h5f:
            for i, image in enumerate(aligned_images):
                # Pastikan format 16-bit tetap terjaga
                h5f.create_dataset(f"image_{i}", data=image.astype(np.uint16), compression="gzip")
                print(language_config.SAVE_TO_HDF5_IMAGE_ALIGNED_SAVING)
    ...
    def run(self):
        try:
            ...
            self.error_signal.emit(language_config.RUN_ERROR_MESSAGE.format(error=str(e)))
...
class ProcessWindows(QWidget):
    def __init__(self, db_path):
        super().__init__()
        self.setWindowTitle(language_config.WINDOW_TITLE_ORB)
    ...
    def start_processing(self):
        ...
        self.processor_thread = OrbAlignment(self.db_path)
