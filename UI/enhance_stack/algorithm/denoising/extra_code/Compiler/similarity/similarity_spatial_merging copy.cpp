...
extern "C"
{
        ...
        float precomputed_ref_noise_sigma)
    {
        ...
        float aggression_factor = 1.0f - std::max(0.0f, std::min(1.0f, (contrast_metric - low_contrast_thresh) / (high_contrast_thresh - low_contrast_thresh)));
        ...
        // =================================================================================
        // === BAGIAN B: ANALISIS SKALA KASAR (Dengan kesadaran-kontras)
        // =================================================================================
        ...
        #pragma omp parallel for schedule(dynamic)
            for (int r_tile = 0; r_tile < num_tiles_h; ++r_tile)
            {
                ...
                for (int c_tile = 0; c_tile < num_tiles_w; ++c_tile)
                {
                    ...
                    const cv::Mat current_tile = current_img_level(roi);
                    const cv::Mat ref_tile = ref_img_level(roi);

                    MotionMatching::TileMatchResult res = MotionMatching::calculate_tile_similarity(
                        current_tile, ref_tile, global_estimated_noise_sigma,
                    ...
                    const int x2 = roi.x + roi.width, y2 = roi.y + roi.height;
        ...
        // =================================================================================
        // === TAHAP 2: ANALISIS SKALA HALUS & AKUMULASI
        // =================================================================================
        ...
        #pragma omp for collapse(2) schedule(dynamic)
            for (int i = 0; i < num_row_starts; i++)
            {
                for (int j = 0; j < num_col_starts; j++)
                {
                    ...
                    const cv::Mat current_tile_fine = current_image_gray_full(tile_roi);
                    const cv::Mat ref_tile_fine = reference_image_gray_full(tile_roi);

                    MotionMatching::TileMatchResult mbm_result = MotionMatching::calculate_tile_similarity(
                        current_tile_fine, ref_tile_fine, global_estimated_noise_sigma,
                    ...
                    float confidence_fine = mbm_result.success ? MotionMatching::calculate_match_confidence(
                                                                     mbm_result, // <--- Argumen kontras dihapus
                                                                     global_estimated_noise_sigma,
                                                                     adapted_motion_sensitivity, 
                                                                     adapted_noise_offset_factor)
                                                               : 0.0f;
