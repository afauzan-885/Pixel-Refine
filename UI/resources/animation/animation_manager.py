...
class StackedWidgetAnimator(QObject):
    ...
    def transition_out(self, widget: QWidget, duration: int = 300,
        ...
        if not widget_ref or not widget_ref():
            if on_finished_callback: QTimer.singleShot(0, on_finished_callback)
            return
    ...
    def transition_in(self, stack_widget: QStackedWidget, target,
        ...
        self._active_transitions[stack_widget] = {
            ...
            'duration_in': duration_in, 'curve_in': curve_in,
        }
        ...
        fade_out.setDuration(duration_out); fade_out.setEasingCurve(curve_out)
        fade_out.setStartValue(1.0); fade_out.setEndValue(0.0)
        out_group.addAnimation(fade_out)
        
        geom_anim = self._create_outgoing_geometry_animation(old_widget, animation_type, duration_out, curve_out)
        if geom_anim: 
            out_group.addAnimation(geom_anim)

        # Hubungkan sinyal menggunakan weakref
        out_group.finished.connect(lambda sw_ref=weakref.ref(stack_widget): self._on_animation_out_finished(sw_ref))
        ...
        if not (data := self._active_transitions.get(stack_widget)) or not data['old_widget_ref']():
    ...
    def _create_outgoing_geometry_animation(self, widget, anim_type, duration, curve):
        ...
        if prop_name:
            ...
            geom_anim.setDuration(duration); geom_anim.setEasingCurve(curve); 
            geom_anim.setStartValue(start_value); geom_anim.setEndValue(end_value)
        return geom_anim
    ...
    def _on_animation_out_finished(self, stack_widget_ref: weakref.ref):
        ...
        if old_widget:
            try:
                old_widget.setGraphicsEffect(None); old_widget.hide()
        ...
        if new_widget:
            ...
            except RuntimeError: del self._active_transitions[stack_widget]
        else:
            del self._active_transitions[stack_widget]

    def _start_incoming_animation(self, stack_widget: QStackedWidget, new_widget: QWidget, data: dict):
        try:
        ...
        fade_in.setDuration(data['duration_in']); fade_in.setEasingCurve(data['curve_in'])
        fade_in.setStartValue(0.0); fade_in.setEndValue(1.0)
        in_group.addAnimation(fade_in)
        
        geom_anim = self._create_incoming_geometry_animation(new_widget, data['animation_type'], data['duration_in'], data['curve_in'])
        if geom_anim: in_group.addAnimation(geom_anim)
        
        in_group.finished.connect(lambda sw_ref=weakref.ref(stack_widget): self._on_animation_in_finished(sw_ref))
        
        if not (active_data := self._active_transitions.get(stack_widget)) or not active_data['new_widget_ref']():
            del self._active_transitions[stack_widget]
            return
...
class HeightAnimator(QObject):
    ...
    def animate_height(self,
        ...
        min_anim.setDuration(duration); min_anim.setEasingCurve(curve)
        min_anim.setStartValue(start_height); min_anim.setEndValue(end_height)
        animation_group.addAnimation(min_anim)

        max_anim = QPropertyAnimation(target_widget, b"maximumHeight", animation_group)
        max_anim.setDuration(duration); max_anim.setEasingCurve(curve)
        max_anim.setStartValue(start_height); max_anim.setEndValue(end_height)
        animation_group.addAnimation(max_anim)

        animation_group.finished.connect(
            lambda w_ref=weakref.ref(target_widget): self._on_height_animation_finished(w_ref)
        )
