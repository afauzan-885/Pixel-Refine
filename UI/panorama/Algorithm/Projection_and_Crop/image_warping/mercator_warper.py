...
class MercatorWarper(BaseWarper):
    # (Metode __init__ dan build_task_list tidak berubah)
    ...
    def warp_and_mask_layer(self, image: np.ndarray, task_info: Dict, image_index: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 1. Ubah koordinat kanvas 2D menjadi sudut longitude (lon) dan latitude (lat)
        lon = (x - task_info['canvas_center_x']) / f
        lat = (y - task_info['canvas_center_y']) / f

        # 2. Buat titik-titik 3D di permukaan bola (unit sphere)
