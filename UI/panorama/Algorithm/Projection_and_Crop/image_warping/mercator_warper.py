...
class MercatorWarper(BaseWarper):
    # (Struktur kelas sama persis dengan CylindricalWarper)
    ...
    def build_task_list(self, output_shape: tuple, tile_size: tuple) -> List[Dict]:
        # ... (Kode ini sama persis dengan CylindricalWarper)
        all_tasks = []
    ...
    def warp_and_mask_layer(self, image: np.ndarray, task_info: Dict, image_index: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if image_index not in self.K_cache:
            self.K_cache[image_index] = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]])
        K = self.K_cache[image_index]
        ...
        # ### RUMUS YANG BENAR UNTUK MERCATOR (PROYEKSI BOLA) ###
        lon = (x - task_info['canvas_center_x']) / f
        lat = np.arctan(np.sinh((y - task_info['canvas_center_y']) / f))

        x_3d = np.cos(lat) * np.sin(lon)
        y_3d = np.sin(lat)
        z_3d = np.cos(lat) * np.cos(lon)
        
        p_3d = np.stack([x_3d, y_3d, z_3d], axis=-1).reshape(-1, 3)
        p_cam = (R_inv @ p_3d.T).T.reshape(tile_h, tile_w, 3)
        
        # (Sisa kode sama persis dengan CylindricalWarper)
        mask = p_cam[..., 2] > 1e-6
        u = np.where(mask, p_cam[..., 0] / p_cam[..., 2] + w/2, -1)
        v = np.where(mask, p_cam[..., 1] / p_cam[..., 2] + h/2, -1)
