...
object SharpnessMetric {
    ...
    fun computeSharpness(buffer: ByteBuffer, width: Int, height: Int): Double {
        if (width < 3 || height < 3 || buffer.remaining() < width * height) return 0.0
        ...
        for (y in 1 until height - 1 step 4) {
            for (x in 1 until width - 1 step 4) {
                val center = buffer.get(y * rowStride + x).toInt() and 0xFF
                val up = buffer.get((y - 1) * rowStride + x).toInt() and 0xFF
                val down = buffer.get((y + 1) * rowStride + x).toInt() and 0xFF
                val left = buffer.get(y * rowStride + (x - 1)).toInt() and 0xFF
                val right = buffer.get(y * rowStride + (x + 1)).toInt() and 0xFF
